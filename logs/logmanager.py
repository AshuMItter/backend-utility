...
# file_handler = logging.FileHandler('app.json.log',encoding='utf-8')
...
class JsonSecureLogger:
    def __init__(self, logger_name='secure_json_logger', log_file='app.json.log'):  
        self.logger = logging.getLogger(logger_name)
        self.logger.setLevel(logging.INFO)

        if not self.logger.handlers is []:
            self.logger.handlers.clear()  # Clear existing handlers to avoid duplicate logs
            # Clear existing handlers to avoid duplicate logs
    

        # Create a file handler for logging to a JSON file
        self.file_handler = logging.FileHandler(log_file, encoding='utf-8')
        self.file_handler.setFormatter(JSONFormatter())
        self.file_handler.addFilter(JSONFilter())

        # Add the file handler to the logger
        self.logger.addHandler(self.file_handler)

    def logCustomLog(self, level, message, data=None):
        if data is None:
            data = {}
